...
const TransactionConfirmationModal: React.FC<TransactionConfirmationModalProps> = ({ onClose }) => {
  const { approvalStatus, transferStatus, sourceNetwork, transferTxHash } = useInitialization();

  let icon = <CircleXIcon />;
  let link = "";

  if (approvalStatus === "success" || transferStatus === "success") {
    icon = <UpArrowIcon />;
    link = `${sourceNetwork?.blockExplorer}/tx/${transferTxHash}`;
  }
    ...
    <GenericModal title="" onClose={onClose}>
      <div className="flex flex-col h-full text-center">
        ...
        <p className="mb-8 text-md">
          ...
          ${sourceNetwork?.name}.`}
          {!link && "An error has occurred, Please start over and try again."}
        </p>
        {link && (
        ...
        <div className="mt-auto">
          <ActionButton title="OK" handler={onClose} disabled={false} errored={false} />
        </div>
      </div>
    </GenericModal>
  );
